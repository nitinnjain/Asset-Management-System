...
<body>
	...
	%>
	<%
		List<List<AssetSerialNumberEntity>> assetList = null;
	%>
			...
			<ul class="nav navbar-nav">
				...
				<li class="active"><a
					href="/AssetManagementSystem/ManagerController?operation=my_team_assets">My
						Team's Assets</a></li>
				<li><a href="/AssetManagementSystem/ManagerJsp/techSupport.jsp">Need
	...
	<table class="table table-striped table-hover">
		...
		<%
				if (request.getAttribute("assetList") != null) {
					assetList = (List<List<AssetSerialNumberEntity>>) request.getAttribute("assetList");
					for (List<AssetSerialNumberEntity> al : assetList) {
						for (AssetSerialNumberEntity asset : al) {
			%>
		<tr>
			<td><%=asset.getEmployeeId()%></td>
			<td><%=asset.getAssetSerialNumber()%></td>
			<td><%=asset.getAssetPool().getAssetName()%></td>
			<td><%=asset.getAllocatedBy()%></td>
		</tr>
		<%
				}
					}
